        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-slate-800 rounded-lg shadow-xl p-6 border border-slate-700 w-full max-w-lg">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-slate-100">Add New Domain Card</h2>
                    <button onClick={onClose} className="text-slate-400 hover:text-white text-3xl leading-none">&times;</button>
                </div>
                <div className="space-y-4">
                    <p className="text-slate-400">Choose a new domain card of your level or lower from your available domains.</p>
                    <select value={selectedCard} onChange={e => setSelectedCard(e.target.value)} className="w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-slate-200">
                         ...
                         {availableCards.map(c => <option key={c.name} value={c.name}>{c.name} (Lvl {c.level}, {c.domain})</option>)}
                     </select>
                </div>
