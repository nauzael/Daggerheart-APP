    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" aria-modal="true" role="dialog">
            <div className="bg-slate-800 rounded-lg shadow-xl p-4 sm:p-6 border border-slate-700 w-full max-w-4xl flex flex-col max-h-[90vh]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-2 gap-6 overflow-hidden flex-grow">
                    {/* List Column - Conditional visibility on mobile */}
                    <div className={`md:col-span-1 lg:col-span-1 overflow-y-auto pr-2 space-y-2 ${isShowingDetailsOnMobile ? 'hidden md:block' : 'block'}`}>
                        {items.map(item => (
                            <button
                                key={item.name}
                                onClick={() => {
                    ...
                    <div className={`md:col-span-2 lg:col-span-1 bg-slate-900/50 p-4 rounded-lg overflow-y-auto ${isShowingDetailsOnMobile ? 'block' : 'hidden md:block'}`}>
                        <button 
